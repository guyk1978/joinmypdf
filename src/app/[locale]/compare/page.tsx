...
function JoinMyPdfCell({ children }: { children: ReactNode }) {
  return (
    <td className="border-b border-slate-100 bg-neutral-900/40 p-4 align-top text-black dark:text-neutral-200 dark:border-slate-800/60 dark:bg-neutral-900/20 dark:text-slate-200 md:p-3">
      <span className="flex items-start gap-2.5">
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl space-y-10 px-4 py-10 md:px-4">
        <header className="space-y-2">
          ...
          <p className="text-black dark:text-neutral-300 dark:text-slate-300">
            {locale === "en" ? (
              <>
                JoinMyPDF is not trying to clone every feature of giant PDF suites. We optimize for{" "}
                <span className="font-semibold text-black dark:text-neutral-200 dark:text-slate-100">{t("introPrivacy")}</span>,{" "}
                <span className="font-semibold text-black dark:text-neutral-200 dark:text-slate-100">{t("introUx")}</span>, and{" "}
                <span className="font-semibold text-black dark:text-neutral-200 dark:text-slate-100">{t("introSpeed")}</span>.
              </>
            ) : (
              t("intro")
            )}
        ...
        <div className="overflow-x-auto rounded-none border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="text-sm text-black dark:text-neutral-300 dark:text-slate-300">
          {t("ctaPrefix")}{" "}
