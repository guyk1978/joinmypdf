...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "JsonToCsvPage" });
...
export default async function JsonToCsvPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("JsonToCsvPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureFlatten"),
            t("schemaFeatureHeaders"),
