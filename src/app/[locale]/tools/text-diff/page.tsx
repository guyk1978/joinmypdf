...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "TextDiffPage" });
...
export default async function TextDiffPage({ params }: PageProps) {
  ...
  const t = await getTranslations("TextDiffPage");
    ...
    <>
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="text-diff-article">
            <h2
              id="text-diff-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section key={section} className="mb-10 last:mb-0" aria-labelledby={`text-diff-article-${section}`}>
                  <h3
                    id={`text-diff-article-${section}`}
