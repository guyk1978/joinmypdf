...
const SLUG = "csv-to-markdown-table";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "CsvMarkdownPage" });
...
export default async function CsvMarkdownPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("CsvMarkdownPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureParse"),
            t("schemaFeatureAlign"),
            t("schemaFeaturePipe"),
