...
const ARTICLE_SECTIONS = ["history", "designDev", "typography"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "LoremIpsumGeneratorPage" });
...
export default async function LoremIpsumGeneratorPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("LoremIpsumGeneratorPage");
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          <div className="tool-page-layout__breadcrumbs">
            <ToolBreadcrumbs
              tool={{ slug: SLUG, title: t("title"), category: "productivity" }}
          ...
          <article
            ...
            aria-labelledby="lorem-ipsum-generator-article"
          >
            <h2
              id="lorem-ipsum-generator-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`lorem-ipsum-generator-article-${section}`}
                >
                  <h3
                    id={`lorem-ipsum-generator-article-${section}`}
