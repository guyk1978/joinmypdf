...
const SLUG = "readability-analyzer";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["whyClarity", "howToUse", "seoAccess", "privacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "ReadabilityAnalyzerPage" });
...
export default async function ReadabilityAnalyzerPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("ReadabilityAnalyzerPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureScore"),
            t("schemaFeatureComplexity"),
            t("schemaFeatureSimplify"),
            t("schemaFeatureTone"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="readability-analyzer-article">
            <h2
              id="readability-analyzer-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`readability-analyzer-article-${section}`}
                >
                  <h3
                    id={`readability-analyzer-article-${section}`}
