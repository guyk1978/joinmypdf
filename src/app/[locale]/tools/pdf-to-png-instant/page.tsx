...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "PdfToPngInstantPage" });
...
export default async function PdfToPngInstantPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("PdfToPngInstantPage");
  ...
  const relatedTools = [
    ...
    { href: "/tools/png-to-pdf/", label: t("relatedPngToPdf") },
...
<section className="border-b border-[#262626] pb-8" aria-label={t("title")}>
          ...
          <section
            ...
            aria-labelledby="pdf-to-png-instant-related"
          >
            <h2
              id="pdf-to-png-instant-related"
