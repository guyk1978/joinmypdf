...
const ARTICLE_SECTIONS = ["webp", "compatibility", "localFirst"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "ImageConverterPage" });
...
export default async function ImageConverterPage({ params }: PageProps) {
  ...
  const t = await getTranslations("ImageConverterPage");
    ...
    <>
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="image-converter-article">
            <h2
              id="image-converter-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`image-converter-article-${section}`}
                >
                  <h3
                    id={`image-converter-article-${section}`}
