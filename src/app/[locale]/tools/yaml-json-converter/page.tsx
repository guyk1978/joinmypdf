...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "YamlJsonConverterPage" });
...
export default async function YamlJsonConverterPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("YamlJsonConverterPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureBidirectional"),
            t("schemaFeatureIndent"),
