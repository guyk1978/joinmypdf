...
const ARTICLE_SECTIONS = ["hex", "rgb", "hsl", "useCases", "howToUse"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "ColorConverterPage" });
...
export default async function ColorConverterPage({ params }: PageProps) {
  ...
  const t = await getTranslations("ColorConverterPage");
    ...
    <>
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="color-converter-article">
            <h2
              id="color-converter-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`color-converter-article-${section}`}
                >
                  <h3
                    id={`color-converter-article-${section}`}
