...
const SLUG = "storage-data-converter";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["howToUse", "decimalVsBinary", "developers", "privacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "StorageDataConverterPage" });
...
export default async function StorageDataConverterPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("StorageDataConverterPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureUnits"),
            t("schemaFeatureLive"),
            t("schemaFeatureSiIec"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="storage-data-article">
            <h2
              id="storage-data-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`storage-data-article-${section}`}
                >
                  <h3
                    id={`storage-data-article-${section}`}
