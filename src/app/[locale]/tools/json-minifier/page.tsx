...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "JsonMinifierPage" });
...
export default async function JsonMinifierPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("JsonMinifierPage");
