...
const SLUG = "image-blur-redact";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["howToUse", "protectScreenshots", "modes", "privacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "ImageBlurRedactPage" });
...
export default async function ImageBlurRedactPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("ImageBlurRedactPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          featureList: [
            t("schemaFeatureBlur"),
            t("schemaFeaturePixelate"),
            t("schemaFeatureSolid"),
            t("schemaFeatureFaces"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="image-blur-redact-article">
            <h2
              id="image-blur-redact-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`image-blur-redact-article-${section}`}
                >
                  <h3
                    id={`image-blur-redact-article-${section}`}
