...
const SLUG = "json-csv-explorer";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["audits", "nestedData", "howToUse", "privacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "JsonCsvExplorerPage" });
...
export default async function JsonCsvExplorerPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("JsonCsvExplorerPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureViewer"),
            t("schemaFeatureSearch"),
            t("schemaFeatureFilter"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="json-csv-explorer-article">
            <h2
              id="json-csv-explorer-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`json-csv-explorer-article-${section}`}
                >
                  <h3
                    id={`json-csv-explorer-article-${section}`}
