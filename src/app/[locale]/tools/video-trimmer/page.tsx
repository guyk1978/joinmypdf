...
const SLUG = "video-trimmer";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["secureTrim", "codecs", "clientSide"] as const;
...
export default async function VideoTrimmerPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("VideoTrimmerPage");
    ...
    <>
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="video-trimmer-article">
            <h2
              id="video-trimmer-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`video-trimmer-article-${section}`}
                >
                  <h3
                    id={`video-trimmer-article-${section}`}
