...
const ARTICLE_SECTIONS = ["qualityCompress", "bitrateCrf", "localSafe"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "VideoCompressorPage" });
...
export default async function VideoCompressorPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("VideoCompressorPage");
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article
            ...
            aria-labelledby="video-compressor-article"
          >
            <h2
              id="video-compressor-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`video-compressor-article-${section}`}
                >
                  <h3
                    id={`video-compressor-article-${section}`}
