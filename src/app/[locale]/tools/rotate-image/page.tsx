...
const ARTICLE_SECTIONS = ["howToUse", "deskewScans", "precisionFlip", "privacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "RotateAlignSuitePage" });
...
export default async function RotateAlignSuitePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("RotateAlignSuitePage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureQuick"),
            t("schemaFeaturePrecision"),
            t("schemaFeatureFlip"),
            t("schemaFeatureDeskew"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="rotate-align-article">
            <h2
              id="rotate-align-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`rotate-align-article-${section}`}
                >
                  <h3
                    id={`rotate-align-article-${section}`}
