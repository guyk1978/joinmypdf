...
const ARTICLE_SECTIONS = ["efficientConvert", "qualityVsSize", "clientSide"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "VideoToGifPage" });
...
export default async function VideoToGifPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("VideoToGifPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureConvert"),
            t("schemaFeatureControls"),
            t("schemaFeaturePalette"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="video-to-gif-article">
            <h2
              id="video-to-gif-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`video-to-gif-article-${section}`}
                >
                  <h3
                    id={`video-to-gif-article-${section}`}
