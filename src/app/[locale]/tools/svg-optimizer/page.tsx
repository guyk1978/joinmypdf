...
const ARTICLE_SECTIONS = ["performance", "metadata", "svgo"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "SvgOptimizerPage" });
...
export default async function SvgOptimizerPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("SvgOptimizerPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureOptimize"),
            t("schemaFeatureCompare"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          <div className="tool-page-layout__breadcrumbs">
            <ToolBreadcrumbs
              tool={{ slug: SLUG, title: t("title"), category: "optimize" }}
              category="optimize"
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="svg-optimizer-article">
            <h2
              id="svg-optimizer-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`svg-optimizer-article-${section}`}
                >
                  <h3
                    id={`svg-optimizer-article-${section}`}
