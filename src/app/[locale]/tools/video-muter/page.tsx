...
const SLUG = "video-muter";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["removeAudio", "socialMute", "localSecurity"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "VideoMuterPage" });
...
export default async function VideoMuterPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("VideoMuterPage");
    ...
    <>
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="video-muter-article">
            <h2
              id="video-muter-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`video-muter-article-${section}`}
                >
                  <h3
                    id={`video-muter-article-${section}`}
