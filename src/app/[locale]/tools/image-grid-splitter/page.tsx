...
const SLUG = "image-grid-splitter";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["howToUse", "instagram", "quality", "privacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "ImageGridSplitterPage" });
...
export default async function ImageGridSplitterPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("ImageGridSplitterPage");
    ...
    <>
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article className="border-b border-[#262626] py-10" aria-labelledby="image-grid-splitter-article">
            <h2
              id="image-grid-splitter-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`image-grid-splitter-article-${section}`}
                >
                  <h3
                    id={`image-grid-splitter-article-${section}`}
