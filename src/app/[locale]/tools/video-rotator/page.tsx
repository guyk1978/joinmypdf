...
const SLUG = "video-rotator";
const PAGE_PATH = `/tools/${SLUG}/`;

type PageProps = { params: Promise<{ locale: string }> };

const ARTICLE_SECTIONS = ["rotateOnline", "mobileOrientation", "localPrivacy"] as const;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "VideoRotatorPage" });
...
export default async function VideoRotatorPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("VideoRotatorPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureAngles"),
            t("schemaFeatureMetadata"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page mx-auto w-full max-w-7xl px-4 md:px-6">
          ...
          <article
            ...
            aria-labelledby="video-rotator-article"
          >
            <h2
              id="video-rotator-article"
            ...
            {ARTICLE_SECTIONS.map((section) => {
              ...
              return (
                <section
                  ...
                  aria-labelledby={`video-rotator-article-${section}`}
                >
                  <h3
                    id={`video-rotator-article-${section}`}
