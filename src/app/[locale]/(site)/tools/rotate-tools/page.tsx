...
import {
  buildRotateToolGridItems,
  getRotateToolFeatureLabels,
  ROTATE_TOOLS_HUB_PATH,
} from "@/lib/rotate-tools";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "RotateToolsPage" });

  const title = t("metaTitle");
  const description = t("metaDescription");
  const canonicalPath = `/${locale}${ROTATE_TOOLS_HUB_PATH}`;
  return {
    title,
    description,
    ...
    alternates: {
      ...
      languages: Object.fromEntries(
        routing.locales.map((item) => [item, `/${item}${ROTATE_TOOLS_HUB_PATH}`]),
      ),
    },
  };
}

export default async function RotateToolsHubPage({ params }: PageProps) {
  const { locale } = await params;
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absoluteUrl(`/${locale}${ROTATE_TOOLS_HUB_PATH}`),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page tools-directory-page--hub-split page-container">
          <CategoryHubPageHeader
            categoryId="rotate"
            title={t("title", { count: getCategoryToolCount("rotate") })}
          ...
          <CategoryHubSplit
            content={
              <>
              <CategorySeoSection categoryId="rotate" />
              </>
            }
            tools={
              <>
              <section className="tools-hub-panel category-hub-split__group" aria-label={t("schemaName")}>
                          <CategoryDirectoryFlatGrid items={gridItems} categoryId="rotate" />
                        </section>
              </>
            }
          />
