...
const SLUG = "color-palette-studio";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "ColorPaletteStudioPage" });
...
export default async function ColorPaletteStudioPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const t = await getTranslations("ColorPaletteStudioPage");
    ...
    <>
      <JsonLd
        data={webApplicationLd({
          ...
          locale,
          featureList: [
            t("schemaFeatureHarmony"),
            t("schemaFeatureExtract"),
            t("schemaFeatureContrast"),
      ...
      <AppPageShell mainClassName={productPageMainClassName}>
        <div className="home-minimal-layout home-minimal-layout--directory tools-directory-page page-container">
          ...
          <FaqSection faqs={faqs} heading={t("faqTitle")} />
        </div>
      </AppPageShell>
    </>
  );
