...
function progressLabel(progress: PdfToWordProgress | null): string {
  if (!progress) return "";
  if (progress.phase === "loading") return "Loading PDF…";
  if (progress.phase === "building") return "Building Word document…";
...
export function PdfToWordWorkspace({ tool, slug }: { tool: ToolDefinition; slug: string }) {
  ...
  const pickFile = async (next: File) => {
    ...
    try {
      ...
      setStatus(`${next.name} ready — convert to Word when you are.`);
  ...
  const onConvert = async () => {
    ...
    try {
      ...
      setDocxBlob(blob);
      setDone(true);
      setStatus("Conversion complete. Download your Word document below.");
  ...
  const onDownload = () => {
    if (!file || !docxBlob) return;
    downloadBlob(docxBlob, pdfToWordOutputName(file));
    capture(EVENTS.download_click, { operation: tool.operation, slug, format: "docx" });
  };

  const showWorkspace = Boolean(file);
  const canConvert = Boolean(file) && !busy;
  const hasDocx = Boolean(docxBlob);
  ...
  return (
    <div id="tool-workspace" className="space-y-6 pb-24 md:pb-8">
      <div className="privacy-callout" role="note">
        <strong>100% Secure:</strong> PDF parsing and Word export run entirely in your browser. Your
      ...
      {!showWorkspace ? (
        <FileUploadZone
          ...
          description="Extract text and download an editable .docx file—processed locally."
      ...
      {showWorkspace ? (
        <div className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.02] p-5 md:p-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              {hasDocx ? "Convert again" : "Convert to Word"}
            </button>
            {hasDocx ? (
              <button
                type="button"
          ...
          {hasDocx && file ? (
            <p className="text-sm text-ink-muted">
              Ready: <span className="font-medium text-ink">{pdfToWordOutputName(file)}</span>
              {docxBlob ? ` (${formatBytes(docxBlob.size)})` : ""}
            </p>
          ) : (
            <p className="text-sm text-ink-muted">
