...
import {
  linearizePdfFromFile,
  pdfLinearizationOutputName,
  type PdfLinearizeProgress,
} from "@/lib/pdf-linearize";
...
function progressPercent(progress: PdfLinearizeProgress | null, busy: boolean): number {
  if (!progress || progress.totalPages <= 0) {
    return busy ? 12 : 0;
  }

  const phaseWeight =
    progress.phase === "loading" ? 0.12 : progress.phase === "optimizing" ? 0.75 : 1;
  const pageRatio = progress.currentPage / progress.totalPages;
  return Math.min(100, Math.round((phaseWeight * 0.3 + pageRatio * 0.7) * 100));
}

export function PdfLinearizationWorkspace({ tool, slug }: { tool: ToolDefinition; slug: string }) {
  const ws = useWorkspaceI18n(tool.operation);
  const labelProgress = (p: PdfLinearizeProgress | null) =>
    progressLabelFromPhase(tool.operation, p, ws);
  const [file, setFile] = useState<File | null>(null);
  const [pageCount, setPageCount] = useState(0);
  const [password, setPassword] = useState("");
  const [status, setStatus] = useState("");
  const [progress, setProgress] = useState<PdfLinearizeProgress | null>(null);
  ...
  const onConvert = async () => {
    ...
    try {
      const bytes = await linearizePdfFromFile(file, {
      ...
      const outName = pdfLinearizationOutputName(file);
