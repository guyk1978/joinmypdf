...
import {
  imageGrayscale,
  imageGrayscaleDownloadName,
  imageGrayscaleZip,
  isAcceptedImageFile,
  type ImageGrayscaleOutput,
  type ImageGrayscaleProgress,
} from "@/lib/image-grayscale";
...
function progressPercent(progress: ImageGrayscaleProgress | null, busy: boolean): number {
...
export function ImageGrayscaleWorkspace({ tool, slug }: { tool: ToolDefinition; slug: string }) {
  const ws = useWorkspaceI18n(tool.operation);
  const t = useTranslations("ImageGrayscale");
  const [files, setFiles] = useState<File[]>([]);
  const [outputs, setOutputs] = useState<ImageGrayscaleOutput[]>([]);
  const [status, setStatus] = useState("");
  const [progress, setProgress] = useState<ImageGrayscaleProgress | null>(null);
  ...
  const onConvert = async () => {
    ...
    try {
      const converted = await imageGrayscale(files, (p) => {
        setProgress(p);
  ...
  const onDownload = async () => {
    ...
    try {
      const zip = await imageGrayscaleZip(outputs);
      downloadBlob(zip, imageGrayscaleDownloadName(outputs));
    } finally {
      setBusy(false);
      setProgress(null);
      setStatus(ws.wsStatus("complete"));
        ...
        <div id={WORKSPACE_OPERATIONS_ID} className="tool-workspace-panel space-y-2">
          ...
          {hasOutput ? (
            <p className="text-sm text-ink-muted">
              ...
              <span className="font-medium text-ink">{imageGrayscaleDownloadName(outputs)}</span>
            </p>
          ) : (
