...
import {
  convertWavFileToMp3,
  mp3OutputFileName,
} from "@/components/tools/ffmpeg/convert-wav-to-mp3";
import {
...
export type FfmpegWavToMp3Result = {
  blob: Blob;
  fileName: string;
};

export type UseFfmpegWavToMp3Options = {
  onComplete?: (result: FfmpegWavToMp3Result) => void;
};

export function useFfmpegWavToMp3(options: UseFfmpegWavToMp3Options = {}) {
  ...
  const [result, setResult] = useState<FfmpegWavToMp3Result | null>(null);
  ...
  const convert = useCallback(async (file: File) => {
    ...
    try {
      const blob = await convertWavFileToMp3(file, {
        onPhase: (nextPhase) => {
          setPhase(nextPhase);
          setStatusMessage(
            nextPhase === "loading" ? "Loading FFmpeg engine…" : "Converting WAV to MP3…",
          );
        },
        onProgress: (progress) => {
          setPhase("processing");
