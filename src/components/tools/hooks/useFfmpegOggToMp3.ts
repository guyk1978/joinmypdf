...
import {
  convertOggFileToMp3,
  mp3OutputFileName,
} from "@/components/tools/ffmpeg/convert-ogg-to-mp3";
import {
...
export type FfmpegOggToMp3Result = {
  blob: Blob;
  fileName: string;
};

export type UseFfmpegOggToMp3Options = {
  onComplete?: (result: FfmpegOggToMp3Result) => void;
};

export function useFfmpegOggToMp3(options: UseFfmpegOggToMp3Options = {}) {
  ...
  const [result, setResult] = useState<FfmpegOggToMp3Result | null>(null);
  ...
  const convert = useCallback(async (file: File) => {
    ...
    try {
      const blob = await convertOggFileToMp3(file, {
        onPhase: (nextPhase) => {
          setPhase(nextPhase);
          setStatusMessage(
            nextPhase === "loading" ? "Loading FFmpeg engine…" : "Converting OGG to MP3…",
          );
        },
        onProgress: (progress) => {
          setPhase("processing");
