...
import {
  isSvgFile,
  svgToPng,
  svgToPngDownloadName,
  svgToPngZip,
  type SvgToPngOutput,
  type SvgToPngProgress,
} from "@/lib/svg-to-png";
...
function progressPercent(progress: SvgToPngProgress | null, busy: boolean): number {
...
export function SvgToPngWorkspace({ tool, slug }: { tool: ToolDefinition; slug: string }) {
  const ws = useWorkspaceI18n(tool.operation);
  const t = useTranslations("SvgToPng");
  const [files, setFiles] = useState<File[]>([]);
  const [outputs, setOutputs] = useState<SvgToPngOutput[]>([]);
  const [status, setStatus] = useState("");
  const [progress, setProgress] = useState<SvgToPngProgress | null>(null);
  ...
  const addFiles = (incoming: FileList | File[]) => {
    const accepted = Array.from(incoming || []).filter(isSvgFile);
    if (!accepted.length) {
      setStatus(ws.wsStatus("noSvg"));
      return;
    }
    const rejected = Array.from(incoming || []).filter((file) => !isSvgFile(file));
  ...
  const onConvert = async () => {
    ...
    try {
      const converted = await svgToPng(files, (p) => {
        setProgress(p);
  ...
  const onDownload = async () => {
    ...
    try {
      const zip = await svgToPngZip(outputs);
      downloadBlob(zip, svgToPngDownloadName(outputs));
    } finally {
      setBusy(false);
      setProgress(null);
      setStatus(ws.wsStatus("complete"));
