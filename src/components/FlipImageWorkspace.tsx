...
import {
  flipImage,
  flipImageDownloadName,
  flipImageZip,
  isAcceptedImageFile,
  type FlipDirection,
  type FlipImageOutput,
  type FlipImageProgress,
} from "@/lib/flip-image";
...
function progressPercent(progress: FlipImageProgress | null, busy: boolean): number {
...
export function FlipImageWorkspace({ tool, slug }: { tool: ToolDefinition; slug: string }) {
  const ws = useWorkspaceI18n(tool.operation);
  const t = useTranslations("FlipImage");
  const [files, setFiles] = useState<File[]>([]);
  const [direction, setDirection] = useState<FlipDirection>("horizontal");
  const [outputs, setOutputs] = useState<FlipImageOutput[]>([]);
  const [status, setStatus] = useState("");
  const [progress, setProgress] = useState<FlipImageProgress | null>(null);
  ...
  const addFiles = (incoming: FileList | File[]) => {
    ...
    const rejected = Array.from(incoming || []).filter((file) => !isAcceptedImageFile(file));
  ...
  const onConvert = async () => {
    ...
    capture(EVENTS.tool_run_start, { operation: tool.operation, slug, direction });

    try {
      const converted = await flipImage(files, direction, (p) => {
        setProgress(p);
      ...
      capture(EVENTS.tool_run_success, { operation: tool.operation, slug, direction });
      window.setTimeout(() => {
  ...
  const onDownload = async () => {
    ...
    try {
      const zip = await flipImageZip(outputs);
      downloadBlob(zip, flipImageDownloadName(outputs));
    } finally {
      setBusy(false);
      setProgress(null);
      setStatus(ws.wsStatus("complete"));
      ...
      {files.length > 0 ? (
        <div id={WORKSPACE_OPERATIONS_ID} className="tool-workspace-panel space-y-2">
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-ink">{t("directionLabel")}</p>
              ...
              <button
                ...
                className={`rounded-none border px-4 py-2 text-sm font-semibold transition ${
                  direction === "horizontal"
                    ? "border-neutral-300 dark:border-neutral-800 bg-neutral-200 dark:bg-neutral-800 text-black dark:text-neutral-200"
          ...
          {hasOutput ? (
            <p className="text-sm text-ink-muted">
              ...
              <span className="font-medium text-ink">{flipImageDownloadName(outputs)}</span>
            </p>
          ) : (
