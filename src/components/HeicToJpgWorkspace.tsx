...
import {
  heicToJpg,
  heicToJpgDownloadName,
  heicToJpgZip,
  isHeicFile,
  type HeicToJpgOutput,
  type HeicToJpgProgress,
...
function progressPercent(progress: HeicToJpgProgress | null, busy: boolean): number {
...
export function HeicToJpgWorkspace({ tool, slug }: { tool: ToolDefinition; slug: string }) {
  const ws = useWorkspaceI18n(tool.operation);
  const [files, setFiles] = useState<File[]>([]);
  const [outputs, setOutputs] = useState<HeicToJpgOutput[]>([]);
  const [status, setStatus] = useState("");
  const [progress, setProgress] = useState<HeicToJpgProgress | null>(null);
  ...
  const onConvert = async () => {
    ...
    try {
      const converted = await heicToJpg(files, (p) => {
        setProgress(p);
  ...
  const onDownload = async () => {
    ...
    try {
      const zip = await heicToJpgZip(outputs);
      downloadBlob(zip, heicToJpgDownloadName(outputs));
    } finally {
      setBusy(false);
      setProgress(null);
      setStatus(ws.wsStatus("complete"));
