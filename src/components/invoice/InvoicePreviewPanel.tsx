...
function AddressBlock({ lines }: { lines: string }) {
  ...
  if (!parts.length) return <p className="text-sm text-neutral-700 dark:text-neutral-400">—</p>;
  return (
    <address className="not-italic text-sm leading-relaxed text-neutral-800 dark:text-neutral-400">
      {parts.map((line) => (
          ...
          <div className="flex h-full flex-col p-[10mm] sm:p-[12mm] md:p-[14mm]">
            <header className="flex flex-wrap items-start justify-between gap-3 border-b border-neutral-300 dark:border-neutral-800 pb-6">
              <div>
                ...
                {document.from.email ? (
                  <p className="mt-1 text-sm text-neutral-700 dark:text-neutral-400">{document.from.email}</p>
                ) : null}
              </div>
            ...
            <section className="mt-6 grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                {document.to.email ? (
                  <p className="mt-2 text-sm text-neutral-700 dark:text-neutral-400">{document.to.email}</p>
                ) : null}
              </div>
            </section>
            ...
            <section className="mt-4 flex-1">
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="border-b-2 border-neutral-300 dark:border-neutral-800 text-left text-xs uppercase tracking-wide text-slate-400">
                  ...
                  {hasItems ? (
                    document.lineItems.map((item) => (
                      <tr key={item.id} className="border-b border-slate-100">
                        ...
                        <td className="py-2.5 pr-2 text-right tabular-nums text-neutral-800 dark:text-neutral-400">
                          {item.quantity}
                        </td>
                        <td className="py-2.5 pr-2 text-right tabular-nums text-neutral-800 dark:text-neutral-400">
