...
import {
  ...
  breadcrumbLd,
  comparisonArticleLd,
  developerUtilityAppLd,
  faqLd,
  howToLd,
  JsonLd,
  localPerformanceAppLd,
  losslessQualityAppLd,
  privacySecurityAppLd,
  technicalFormatAppLd,
  techArticleLd,
} from "@/lib/schema";
...
export async function BlogArticleContent({
  ...
  const youMightLike = getYouMightAlsoLikePosts(
    post,
    blogRegistry.blog || [],
    keepReadingSlugs,
    4,
  ...
  const primaryToolDef = primarySlug
    ? registry.tools.find((tool) => tool.slug === primarySlug)
    : null;
  ...
  const emitDeveloperSoftware =
    post.contentBlocks?.developerSoftwareSchema && primaryToolDef;
      ...
      {emitTechArticle ? (
        <JsonLd
          data={techArticleLd({
            ...
            about:
              locale === "he"
                ? ["User-Agent", "HTTP", "דפדפנים"]
                : ["User-Agent", "HTTP", "Web browsers"],
          })}
        />
      ) : null}
      <JsonLd
