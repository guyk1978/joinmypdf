"use client";

import { useEffect, useMemo } from "react";
import { useTranslations } from "next-intl";
import { capture, EVENTS } from "@/components/AnalyticsClient";
import { ConvertToPNG, type ConvertToPngLabels } from "@/components/ConvertToPNG";
import { WorkspaceUploadShell } from "@/components/WorkspaceUploadShell";
import { WORKSPACE_OPERATIONS_ID } from "@/lib/workspace-flow";
import type { ToolDefinition } from "@/lib/types";

type ConvertToPngWorkspaceProps = {
  tool: ToolDefinition;
  slug: string;
};

export function ConvertToPngWorkspace({ tool, slug }: ConvertToPngWorkspaceProps) {
  const t = useTranslations("ConvertToPng");

  useEffect(() => {
    capture(EVENTS.tool_view, { slug, operation: tool.operation });
  }, [slug, tool.operation]);

  const labels = useMemo<ConvertToPngLabels>(
    () => ({
      dropTitle: t("dropTitle"),
      dropHint: t("dropHint"),
      selectFile: t("selectFile"),
      selectFileAria: t("selectFileAria"),
      convertInstructions: t("convertInstructions"),
      formatFileInfo: (name, width, height) => t("fileInfo", { name, width, height }),
      convertAndDownload: t("convertAndDownload"),
      converting: t("converting"),
      convertingProgress: t("convertingProgress"),
      invalidFile: t("invalidFile"),
      replaceImage: t("replaceImage"),
    }),
    [t],
  );

  return (
    <WorkspaceUploadShell showPrivacyBadge>
      <div id={WORKSPACE_OPERATIONS_ID} className="crop-image-tool-page">
        <ConvertToPNG labels={labels} />
      </div>
    </WorkspaceUploadShell>
  );
}
